    ...
    <Card className="rounded-xl border shadow-sm hover:shadow-lg  transition duration-300 flex flex-col hover:-translate-y-0.5 w-full h-full">
      ...
      <div className="p-4 flex flex-col flex-1  space-y-3">
        <div className="flex justify-between items-center gap-2">
          <h2 className="font-semibold text-lg md:text-2xl">{course.title}</h2>
        ...
        <p className="text-sm md:text-lg text-gray-600">{course.description}</p>
        ...
        <div className="mt-auto pt-3 space-y-4">
          ...
          <Link href={`/courses/${course.id}`}>
            <Button
              variant="solid"
              className="w-full bg-[#3B7597] text-white hover:opacity-90 py-5"
            >
              View Details
            </Button>
          </Link>
